...
- **`README.md`** is the repo's description. It is read at HEAD and surfaced in
  the UI, and editing the description commits the file — so the description is
  part of the repo's history and syncs with it.
- **`LICENSE`** sits beside it, read-only.
- **Any path segment beginning with `.`** — `.github/`, `.domains`,
  `kb/.drafts/` — is **private**: excluded from fact discovery, so nothing under
  it is indexed, searched, or returned as a fact.

Private means "not discovered as a fact", not "rejected". A dot-path is not
blocked at the storage layer, so these files live, sync and diff like any others
— they simply never enter the corpus.
