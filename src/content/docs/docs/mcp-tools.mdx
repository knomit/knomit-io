...
bytes are. `lens` is reported alongside as the reason that repo was chosen, and
is omitted entirely on a repo-bound binding rather than carried as an empty
field. The prose `summary` is not redundant with the struct — a caller reacts to
