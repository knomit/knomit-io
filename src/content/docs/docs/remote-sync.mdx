...
[ontology](/docs/ontologies/) there, and pushes **that branch alone**. An
...
Neither mode mints a new identity: the repo id is the remote's existing root
commit, so two machines pointed at the same remote converge on one identity
