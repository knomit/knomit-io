...
(MCP proxy + Claude integration), and `knomit-desktop` (the app). All three print
the same build string, but the accepted spelling differs: `knomit` and
`knomit-bridge` take only the bare `version` subcommand — `--version` is an
unknown flag on both. `knomit-desktop` accepts `version`, `--version` and
`-version`.
