...
<Aside type="caution" title="No ontology, no writes">
A repository is a knomit knowledge base **if and only if** it has an ontology.
If the file is missing, unreadable, or fails to parse, knomit substitutes
nothing — it logs an error and the repo opens **read-only**. Every branch is
refused for writing, the agent branch included, so no fact is ever authored
against a taxonomy nobody chose.

Existing facts stay readable, searchable and exportable; what you lose is the
write path until the file is repaired. Because the ontology is read once at
open, repairing it means [fixing the file and reopening the
repo](#choosing-an-ontology).

Unknown top-level keys are a **warning**, not a parse failure, so an ontology
written by a newer knomit still opens as itself rather than being rejected
into read-only. The editor and `POST /api/v1/ontologies:validate` stay strict —
they answer `ok: false` for a key knomit does not declare, because they judge
input you are still able to change.
</Aside>
...
| `initialize` | remote | `ontology_preset` or `ontology_yaml`, committed onto knomit's own branch. `409` if that branch already has one. |
...
<Aside type="tip" title="`initialize` never needs push access to `main`">
It clones the chosen branch, cuts `agent/<host>` from its tip, commits the
ontology there, and pushes **that branch alone**. The consensus branch is never
written, so a protected default branch is not an obstacle — open a merge
request from `agent/<host>` when the knowledge base should become the project's
consensus. A remote with no branches at all is refused, with the instruction
that fixes it: create a `main` first, one commit is enough.
</Aside>
...
in a [guided wizard](/docs/web-ui/#creating-a-repository) that derives the mode
from what it probed, rather than asking you to classify the remote yourself.

### Validating a draft before you commit to it

Three endpoints exist so a draft can be checked without creating anything. The
...
| `GET /api/v1/ontologies/presets` | The built-in presets, with id, name, topic list and rule count. |
...
Use this rather than repo creation as your validation step: unlike a create, it
costs nothing and reports all the problems at once.
...
1. A missing or unparseable ontology makes the repo **read-only**. It is not
   replaced by the default; refused writes and an error log are the signal.
