...
`fact.sources` and `fact.origin` are the two provenance axes, and they support
the rules that make an ontology enforce a *standard of evidence* rather than a
shape. `sources` counts independent corroborations, defaulting to 1; `origin`
records which pipeline minted the fact — `authored` for anything a human or
agent asserted, `distilled` for synthesis-pipeline output, `discovered` for the
emergent facts the [discovery engine](/docs/discovery/) surfaces.

<Aside type="caution" title="`sources` is summed on a merge">
Rules run again on the merged fact after a
[subsumption](/docs/concepts/#subsumption--never-learned-twice), and a merge
**adds** the two facts' source counts. A `fact.sources >= 2` rule therefore
gates the first write of a claim and then passes automatically on every
subsequent one — which is usually what you want from a corroboration rule, but
is not a floor that holds per-write.
</Aside>
...
4. `fact.sources` is **summed on a subsumption merge**, so a corroboration
   floor gates a claim's first write and passes on every one after it.
