...
Existing facts stay readable and searchable; what you lose is the write path
until the file is repaired. The reason is reported rather than left to the log —
the branch payload carries an `ontology_error` a client can show. Because the
ontology is read once at open, repairing it means [fixing the file and reopening
the repo](#choosing-an-ontology).
