...
Inside the knowledge base repo, at:

```text
.knomit/ontology.yaml
```

Two older locations are still **read**, for repos that predate the move —
`.domains/ontology.yaml`, then `domains/ontology.yaml`. The first rung with
content wins, and whatever rung was read is the rung written back to. No
migration runs: a repo is moved by hand, and reading all three is what stops an
unmigrated repo from silently falling onto the default taxonomy.

Facts land under the **ontology root**, `kb/` by default and set by
...
`sources` and `origin` are **not** exposed; both are `undefined` inside a rule.
"Require at least two independent sources" is a rule people reach for, and it
...
4. `sources` and `origin` are **invisible to rules**.
