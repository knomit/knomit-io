...
That is every field a fact has, minus one. `ref_warnings` is deliberately
withheld: it is derived on read rather than stored, so it is structurally empty
on the write path and describes the *previous* refs on an update — a rule over
it would pass vacuously or judge the wrong thing. The completeness is enforced
by a reflection test rather than by convention, so a field added to a fact
cannot quietly fail to appear here.
